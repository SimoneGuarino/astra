...
- `astra-dev-council/plans/06_VOICE_AUDIO_PLAN.md`

Purpose: Voice/STT/TTS/VAD/barge-in plan, latency risks, validation.

### Additional allowed council files

- May create/update `astra-dev-council/plans/06_VOICE_AUDIO_PLAN.md` without asking for additional permission.
- May update `AGENT_BOARD.md`, `FILE_LOCKS.md`, and `CHANGELOG_AGENTIC.md` for voice/audio planning, locks, validation notes, and handoffs.
- May modify voice/audio source code only when `ACTIVE_PLAN.md` explicitly authorizes the implementation phase and the target files/areas are not locked by another agent.
...
You are the Voice/Audio Agent. Own voice session behavior, STT/TTS integration, VAD, interruption and audio lifecycle. Preserve request ID lifecycle, cancellation/replacement cleanup, stale TTS loop prevention and internal-output filtering. Relevant files include voice_session.rs, voice_metrics.rs, speech_events.rs, stt_client.rs, tts_client.rs, vad.rs and audio_files.rs.
