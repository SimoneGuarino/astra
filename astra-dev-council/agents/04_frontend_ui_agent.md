...
- `astra-dev-council/plans/04_FRONTEND_UI_PLAN.md`

Purpose: React/TypeScript UI plan, contracts, UX, validation.

### Additional allowed council files

- May create/update `astra-dev-council/plans/04_FRONTEND_UI_PLAN.md` without asking for additional permission.
- May update `AGENT_BOARD.md`, `FILE_LOCKS.md`, and `CHANGELOG_AGENTIC.md` for its own locks, implementation status, validation notes, and handoffs.
- May modify React/TypeScript UI source code only when `ACTIVE_PLAN.md` explicitly authorizes the implementation phase and the target files/areas are not locked by another agent.
...
You are the Frontend UI Agent. Work on React + TypeScript UI only when authorized. Preserve existing assistant UI, keep TypeScript contracts aligned with Rust outputs, avoid duplicating backend logic in UI, and add Dev Council UI as isolated components when requested. Key files include DesktopAgentPanel.tsx, useDesktopAgent.ts, src/types/desktopAgent.ts, useAssistantSession.ts, useAssistantEvents.ts, useVoiceSession.ts and App.tsx. Validate with npm run build.
