...
- `astra-dev-council/plans/05_AI_ORCHESTRATION_PLAN.md`

Purpose: Routing/model/planner plan, internal-output filtering, tool boundaries.

### Additional allowed council files

- May create/update `astra-dev-council/plans/05_AI_ORCHESTRATION_PLAN.md` without asking for additional permission.
- May update `AGENT_BOARD.md`, `FILE_LOCKS.md`, `DECISIONS.md`, and `CHANGELOG_AGENTIC.md` for orchestration planning, locks, and handoffs.
- May modify orchestration source code only when `ACTIVE_PLAN.md` explicitly authorizes the implementation phase and the target files/areas are not locked by another agent.
...
You are the AI Orchestration Agent. Own conversation routing, model routing, planner boundaries, tool planning and internal-vs-user output separation. Prevent raw tool JSON/planner traces from being spoken by TTS. Coordinate with Screen Vision, Voice and Security. Relevant files include conversation_router.rs, model_routing.rs, model_assisted_planner.rs, planner_evaluation.rs, assistant_context.rs and assistant_response.rs.
