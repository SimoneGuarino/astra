...
impl FilesystemService {
    ...
    pub fn ensure_allowed(&self, path: &Path) -> Result<PathBuf, String> {
        ...
        Err(format!("Path is outside allowed roots: {}", normalized.display()))
    }
    ...
    pub fn write_text(&self, path: &str, content: &str, mode: &str) -> Result<serde_json::Value, String> {
        let resolved = self.ensure_allowed(Path::new(path))?;
        ...
        match mode {
            "append" => {
                use std::io::Write;
                let mut file = fs::OpenOptions::new().create(true).append(true).open(&resolved).map_err(|e| e.to_string())?;
        ...
        Ok(json!({"path": resolved.display().to_string(), "bytes_written": content.len(), "mode": mode}))
    }

    pub fn search(&self, root: Option<&str>, pattern: &str, max_results: usize) -> Result<serde_json::Value, String> {
    ...
    fn walk(&self, root: &Path, needle: &str, max_results: usize, results: &mut Vec<String>) -> Result<(), String> {
        ...
        for entry in fs::read_dir(root).map_err(|e| e.to_string())? {
            ...
            } else if path.file_name().and_then(|n| n.to_str()).map(|name| name.to_lowercase().contains(needle)).unwrap_or(false) {
