...
export type KnowledgePackBuildRequest = {
  enabled?: boolean;
  dry_run?: boolean;
  snapshot_limit?: number;
  max_packs?: number;
  max_nodes_per_pack?: number;
  min_nodes_per_pack?: number;
  max_pack_content_members?: number;
  min_pack_score?: number;
  persist_packs?: boolean;
...
export type KnowledgePackKindCount = {
  kind: string;
  count: number;
};

export type KnowledgePackMember = {
  node_id: string;
  title: string;
  kind: string;
  confidence: number;
...
export type KnowledgePackSummary = {
  ...
  member_count: number;
  tags: string[];
  kind_counts: KnowledgePackKindCount[];
  members: KnowledgePackMember[];
  metadata?: Record<string, unknown>;
};

export type KnowledgePackBuildReceipt = {
  accepted: boolean;
  reason: string;
  started_at: number;
  completed_at: number;
...
export type DeepSearchKnowledgeRefreshReceipt = {
  ...
  autopilot?: DeepSearchKnowledgeAutopilotReceipt | null;
  warnings: string[];
  recommendations: string[];
  metadata?: Record<string, unknown>;
};
...
export type DeepSearchAcceptedSource = {
  ...
  reliability_tier?: DeepSearchSourceReliabilityTier | null;
  reliability_reason?: string | null;
  reliability_signals?: string[];
  reliability_penalties?: string[];
};
