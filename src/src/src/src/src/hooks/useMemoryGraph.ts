...
import type {
  ...
  DeepSearchRequest,
  DeepSearchReceipt,
  DeepSearchKnowledgeAutopilotRequest,
  DeepSearchKnowledgeAutopilotReceipt,
} from "../types/memory";
...
export function useMemoryGraph() {
  ...
  const runMemoryAutopilot = useCallback(
    (request: MemoryAutopilotRequest = {
      ...
      knowledge_autopilot_topic_limit: 6,
      knowledge_autopilot_run_limit: 2,
  ...
  const runDeepSearchKnowledgeAutopilot = useCallback(
    (request: DeepSearchKnowledgeAutopilotRequest = {
      ...
      reason: "memory_graph_deep_search_knowledge_autopilot",
    }) =>
      invoke<DeepSearchKnowledgeAutopilotReceipt>("run_deep_search_knowledge_autopilot", { request }),
    []
  );

  const buildKnowledgePacks = useCallback(
    (request: KnowledgePackBuildRequest = { enabled: true, dry_run: false, snapshot_limit: 500, max_packs: 12, max_nodes_per_pack: 32, min_nodes_per_pack: 3, min_pack_score: 0.34, persist_packs: true, include_unverified: false, include_low_confidence: true, include_source_documents: false }) =>
  ...
  return useMemo(
    () => ({
      ...
      mergeMemoryNodes,
      runDeepSearch,
      runDeepSearchKnowledgeAutopilot,
