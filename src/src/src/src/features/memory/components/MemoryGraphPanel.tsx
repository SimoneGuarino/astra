...
export function MemoryGraphPanel({ mode = "embedded", onClose }: MemoryGraphPanelProps) {
  ...
  const handleRunKnowledgeAutopilot = useCallback(async () => {
    try {
      setError(null);
      setIsLoading(true);
      setMemoryAutopilotStatus("Knowledge Autopilot studying…");
      const receipt = await memory.runDeepSearchKnowledgeAutopilot({
        enabled: true,
        dry_run: false,
        max_topics: 8,
        max_runs: 3,
        ...
        reason: "memory_graph_toolbar_knowledge_autopilot",
      });
      setMemoryAutopilotStatus(
        `Knowledge Autopilot: ${receipt.runs_executed} research runs, ${receipt.sources_accepted} sources, ${receipt.claims_promoted} promoted claims`
      );
      setSkillStatus(
        receipt.recommendations.length > 0
          ? receipt.recommendations.slice(0, 2).join(" · ")
          : `Agenda items: ${receipt.agenda_items}, extracted claims: ${receipt.claims_extracted}`
      );
      await refresh();
      await refreshMemoryControlCenter();
    } catch (err) {
      ...
      {reviewExpanded ? (
        <aside className="memory-merge-overlay memory-autopilot-review-overlay" aria-label="Governed Brain Review queue">
          ...
          <div className="memory-merge-overlay-actions">
            ...
            <button type="button" onClick={() => void handleRunKnowledgeAutopilot()} disabled={isLoading} title="Run bounded deep-search learning over mined memory topics; external knowledge is auto-promoted only as LLM-inferred memory">Run knowledge autopilot</button>
