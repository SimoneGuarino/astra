  ...
  const handleRunMemoryAutopilot = useCallback(async () => {
    try {
      ...
      setMemoryAutopilotStatus(
        `Autopilot: quality ${Math.round(receipt.quality_score * 100)}%, ${receipt.semantic_nodes_created} semantic, ${receipt.embeddings_indexed} vectors, ${autoActions} auto / ${reviewActions} review`
      );
      const reviewCount = receipt.duplicate_candidates + receipt.canonical_review_candidates;
      setSkillStatus(
        `Autopilot completed: ${receipt.skill_candidates} skills, ${receipt.duplicate_candidates} duplicate proposals, ${receipt.canonical_review_candidates} canonical proposals`
      );
      await refresh();
      if (reviewCount > 0) {
